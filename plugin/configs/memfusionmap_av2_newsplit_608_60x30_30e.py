...
# plugin code dir
plugin_dir = 'plugin/'
data_root = '/raid/argoverse2'
pkl_root = '/workspace/jingyus/dataset_pkl/argoverse2/streammapnet'
...
model = dict(
    ...
    bev_w=bev_w,
    use_overlap=True,
    use_overlap_gated=True,
    first_frame_gated=True,
    memory_length=4,
    backbone_cfg=dict(
...
eval_config = dict(
    type='AV2Dataset',
    ann_file=pkl_root+'/av2_map_infos_val_newsplit.pkl',
    meta=meta,
    roi_size=roi_size,
    cat2id=cat2id,
    pipeline=[
...
data = dict(
    ...
    train=dict(
        type='AV2Dataset',
        ann_file=pkl_root+'/av2_map_infos_train_newsplit.pkl',
        meta=meta,
        roi_size=roi_size,
        cat2id=cat2id,
...
evaluation = dict(interval=total_iters//15)
find_unused_parameters = True #### when use checkpoint, find_unused_parameters must be False
checkpoint_config = dict(interval=total_iters//30)
